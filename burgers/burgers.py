...
# Config
# ============================================================
MODE = 'pcgrad'      # 'pinn' | 'pcgrad'
NET_ARCH = 'mlp'     # 'mlp' | 'attention'
...
def sampler():
    ...
    idx_x = np.random.choice(len(x), NUM_0, replace=False)
    x0 = x[idx_x]
    u0 = Exact[idx_x, 0:1]

    idx_t = np.random.choice(len(t), NUM_B, replace=False)
    tb = t[idx_t]
    ...
    X_lb = np.concatenate([-np.ones_like(tb), tb], 1)
    X_ub = np.concatenate([ np.ones_like(tb), tb], 1)
...
# ============================================================
HISTORY_COLS = ['iter', 'pde', 'bc', 'ic', 'total', 'l2', 'linf']
...
def train_single_run(run_idx, seed):
    ...
    X_f, X_lb, X_ub, X0, u0, x, t, Exact = sampler()
    ...
    def forward_losses():
        ...
        L_bc = torch.mean(net(X_lb_norm)**2) + torch.mean(net(X_ub_norm)**2)
        L_ic = torch.mean((net(X0_norm) - u0)**2)
    ...
    for ep in range(EPOCHS):
        ...
        if ep % RECORD_EVERY == 0:
            ...
            history.append([ep, L_pde.item(), L_bc.item(), L_ic.item(), L.item(), l2, linf])
            if l2 < best_l2:
                best_l2 = l2
                best_linf = linf
