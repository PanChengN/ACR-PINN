...
# ============================================================
# Defaults (match klein_gordon.py)
# ============================================================
ALPHA = -1.0
BETA = 0.0
GAMMA = 1.0
...
def plot_outputs(X, T, u_exact, u_pred, fig_dir, prefix=''):
    ...
    for target in SLICE_TIMES:
        ...
        ax.legend()
        fig.tight_layout()
        plt.savefig(os.path.join(fig_dir, f'{slice_prefix}{t_values[idx]:.2f}.pdf'),
        ...
        fig.tight_layout()
        plt.savefig(os.path.join(fig_dir, f'{slice_err_prefix}{t_values[idx]:.2f}.pdf'),
                    dpi=300, bbox_inches='tight', pad_inches=0.02, transparent=True)
        plt.close()
...
def main():
    parser = argparse.ArgumentParser(description='Replot Klein-Gordon results from saved checkpoints.')
    ...
    x = np.linspace(0, 1, args.n_test)
    t = np.linspace(0, 1, args.n_test)
    X, T = np.meshgrid(x, t)
    X_star = np.hstack([X.flatten()[:, None], T.flatten()[:, None]])
    X_star_t = torch.tensor(X_star, dtype=torch.float32, device=device)
    X_star_norm = normalize_xt(X_star_t)
    u_exact = exact_solution(X_star_t).detach().cpu().numpy().reshape(X.shape)
    ...
    for ckpt_path in ckpts:
        ...
        plot_outputs(X, T, u_exact, u_pred, out_dir)
