...
NUM_F = int(os.getenv('KLEIN_GORDON_NUM_F', '10000'))
NUM_B = int(os.getenv('KLEIN_GORDON_NUM_B', '200'))
NUM_IC = int(os.getenv('KLEIN_GORDON_NUM_IC', '200'))
TIME_MAX = float(os.getenv('KLEIN_GORDON_TIME_MAX', '1.0'))
DYNAMIC_SAMPLING = os.getenv('KLEIN_GORDON_DYNAMIC_SAMPLING', '0').strip().lower() in ('1', 'true', 'yes')
RESAMPLE_EVERY = int(os.getenv('KLEIN_GORDON_RESAMPLE_EVERY', '1'))
if MODE in ('rba', 'brdr') and DYNAMIC_SAMPLING:
    raise ValueError(f'{MODE.upper()} requires fixed collocation points; set KLEIN_GORDON_DYNAMIC_SAMPLING=0.')
if TIME_MAX <= 0:
    raise ValueError('KLEIN_GORDON_TIME_MAX must be positive.')
...
GENERATED_DATA_DIR = REPO_ROOT / 'generated_data' / 'klein_gordon'
...
def sampler(sample_seed):
    time_label = f'{TIME_MAX:g}'.replace('.', 'p')
    dataset_path = GENERATED_DATA_DIR / (
        f'seed_{sample_seed}_t{time_label}_nf{NUM_F}_nb{NUM_B}_nic{NUM_IC}.npz'
    )

    def create_dataset():
        lhs_rng, bc_rng, ic_rng = spawned_rngs(sample_seed, 3)
        X_f = latin_hypercube(2, NUM_F, lhs_rng)
        X_f[:, 1:2] *= TIME_MAX
        n_bc = NUM_B // 2
        X_bc = np.vstack([
            np.hstack([np.zeros((n_bc, 1)), TIME_MAX * bc_rng.random((n_bc, 1))]),
            np.hstack([
                np.ones((NUM_B - n_bc, 1)),
                TIME_MAX * bc_rng.random((NUM_B - n_bc, 1)),
            ]),
        ])
        x_ic = ic_rng.random((NUM_IC, 1))
        return {'X_f': X_f, 'X_bc': X_bc,
    ...
    return (
        ...
        torch.tensor(arrays['X_bc'], dtype=torch.float32, device=device),
        torch.tensor(arrays['X_ic'], dtype=torch.float32, device=device, requires_grad=True),
...
def dynamic_sample_epoch(sample_seed, epoch):
    ...
    X_f[:, 1:2] *= TIME_MAX
    boundary_t = TIME_MAX * fast_lhs(rng, NUM_B, 1)
    n_left = NUM_B // 2
    X_bc = np.vstack([
        np.hstack([np.zeros((n_left, 1)), boundary_t[:n_left]]),
        np.hstack([np.ones((NUM_B - n_left, 1)), boundary_t[n_left:]]),
    ])
    x_ic = fast_lhs(rng, NUM_IC, 1)
    X_ic = np.hstack([x_ic, np.zeros((NUM_IC, 1))])
    return (
...
def dynamic_sampling_descriptor(sample_seed):
    ...
    path = GENERATED_DATA_DIR / (
        f'seed_{sample_seed}_t{time_label}_dynamic_nf{NUM_F}_nb{NUM_B}_nic{NUM_IC}_r{RESAMPLE_EVERY}.json'
    )
    return write_sampling_descriptor(path, {
        ...
        'num_boundary_total': NUM_B, 'num_initial': NUM_IC,
        'resample_every': RESAMPLE_EVERY, 'domain': [[0.0, 1.0], [0.0, TIME_MAX]],
    })
...
SLICE_TIME_FRACTIONS = [0.0, 0.25, 0.5, 0.75, 1.0]
EXACT_COLOR = '#2b2b2b'
PRED_COLOR = '#d55e00'
SMOOTH_SIGMA = 3.0
...
def plot_run_outputs(X, T, Exact, x, t, X_star, u_pred, fig_dir, prefix=''):
    ...
    x_values = x.flatten()
    slice_paths = []
    for fraction in SLICE_TIME_FRACTIONS:
        target = fraction * TIME_MAX
...
def time_resolved_metrics(t_values, exact, prediction):
    """Return fixed-test error at each time slice for long-time KG evaluation."""
    rows = []
    for index, time_value in enumerate(t_values):
        exact_slice = exact[index, :]
        prediction_slice = prediction[index, :]
        l2_denominator = np.linalg.norm(exact_slice)
        linf_denominator = np.max(np.abs(exact_slice))
        rows.append({
            'time': float(time_value),
            'normalized_time': float(time_value / TIME_MAX),
            'relative_l2': float(
                np.linalg.norm(prediction_slice - exact_slice) / l2_denominator
                if l2_denominator > 0 else np.nan
            ),
            'relative_linf': float(
                np.max(np.abs(prediction_slice - exact_slice)) / linf_denominator
                if linf_denominator > 0 else np.nan
            ),
            'mse': float(np.mean((prediction_slice - exact_slice) ** 2)),
        })
    return rows


def write_time_metrics(path, rows):
    with open(path, 'w', newline='') as handle:
        writer = csv.DictWriter(
            handle,
            fieldnames=['time', 'normalized_time', 'relative_l2', 'relative_linf', 'mse'],
        )
        writer.writeheader()
...
# ============================================================
HISTORY_COLS = ['iter', 'pde', 'bc', 'ic', 'ic_t', 'total', 'l2', 'linf', 'mse']
...
def train_single_run(run_idx, seeds):
    ...
    if DYNAMIC_SAMPLING:
        ...
        X_f, X_bc, X_ic = dynamic_sample_epoch(seeds['sample_seed'], 0)
    else:
        X_f, X_bc, X_ic, dataset_path, dataset_sha256 = sampler(seeds['sample_seed'])
    ...
    gradnorm_balancer = GradNormBalancer(4, GRADNORM_ALPHA, GRADNORM_LR) if MODE == 'gradnorm' else None
    ntk_generator = torch.Generator().manual_seed(seeds['optimizer_order_seed'] + 3000)
    ntk_weights = torch.ones(4, device=device)
    lra_balancer = GradientStatisticsLossBalancer(4, LRA_EMA) if MODE == 'lra' else None
    lra_weights = torch.ones(4, device=device)
    ...
    config = {'problem': 'klein_gordon', 'mode': MODE, 'network': NET_ARCH,
              ...
              'num_residual': NUM_F, 'num_boundary': NUM_B, 'num_initial': NUM_IC,
              'time_max': TIME_MAX, 'time_slice_fractions': SLICE_TIME_FRACTIONS,
              'alpha': ALPHA, 'beta': BETA, 'gamma': GAMMA, 'power': K_POWER,
    ...
    t = np.linspace(0, TIME_MAX, N_test)
    X, T = np.meshgrid(x, t)
    ...
    def forward_losses(return_task_outputs=False):
        ...
        boundary_error = u_b_pred - u_bc
        L_bc = torch.mean(boundary_error ** 2)

        u_ic_pred = net(normalize_xt(X_ic))
        initial_error = u_ic_pred - u_ic
        L_ic = torch.mean(initial_error ** 2)
        u_t_pred = grad(u_ic_pred, X_ic, torch.ones_like(u_ic_pred), create_graph=True)[0][:, 1:2]
        temporal_error = u_t_pred - ut_ic
        L_ic_t = torch.mean(temporal_error ** 2)

        if return_task_outputs:
            return L_pde, L_bc, L_ic, L_ic_t, [res, boundary_error, initial_error, temporal_error]
        return L_pde, L_bc, L_ic, L_ic_t

    for ep in range(EPOCHS):
        if DYNAMIC_SAMPLING:
            X_f, X_bc, X_ic = dynamic_sample_epoch(seeds['sample_seed'], ep)
            X_bc_norm = normalize_xt(X_bc)
            with torch.no_grad():
                u_bc = exact_solution(X_bc)
                u_ic = exact_solution(X_ic)
        ...
        if MODE == 'ntk' and ep % NTK_UPDATE_EVERY == 0:
            L_pde, L_bc, L_ic, L_ic_t, task_outputs = forward_losses(return_task_outputs=True)
        ...
        task_names = ['pde', 'bc', 'ic', 'ic_t']
        params = list(net.parameters())
        named_params = list(net.named_parameters())
        diagnostic_grads = None
        projected_grads = None
        optimizer_weights = None
        if MODE in ('sum', 'ntk', 'lra', 'rba', 'brdr') and recorder.should_record(ep, EPOCHS):
        ...
        if ep % RECORD_EVERY == 0 or ep == EPOCHS - 1:
            ...
            history.append([ep, L_pde.item(), L_bc.item(), L_ic.item(), L_ic_t.item(), L.item(), l2, linf, mse])
    ...
    cost_profiler.write(run_log_dir, net, {'problem': 'klein_gordon', 'architecture': NET_ARCH, 'optimizer': MODE, 'time_max': TIME_MAX})
    ...
    time_metrics = time_resolved_metrics(
        t, u_star.reshape(X.shape), u_pred.reshape(X.shape)
    )
    write_time_metrics(os.path.join(run_log_dir, 'time_error.csv'), time_metrics)
...
# ============================================================
# Time-resolved long-time errors
# ============================================================
time_metric_rows = []
for result in run_results:
    for row in result['time_metrics']:
        time_metric_rows.append({'run': result['seeds']['master_seed'], **row})
if time_metric_rows:
    aggregate_time_path = os.path.join(AGG_LOG_DIR, 'time_error_summary.csv')
    with open(aggregate_time_path, 'w', newline='') as handle:
        writer = csv.DictWriter(
            handle,
            fieldnames=[
                'time', 'normalized_time', 'relative_l2_mean', 'relative_l2_std',
                'relative_linf_mean', 'relative_linf_std', 'mse_mean', 'mse_std',
            ],
        )
        writer.writeheader()
        for time_value in sorted({row['time'] for row in time_metric_rows}):
            selected = [row for row in time_metric_rows if row['time'] == time_value]
            writer.writerow({
                'time': time_value,
                'normalized_time': selected[0]['normalized_time'],
                'relative_l2_mean': np.mean([row['relative_l2'] for row in selected]),
                'relative_l2_std': np.std([row['relative_l2'] for row in selected]),
                'relative_linf_mean': np.mean([row['relative_linf'] for row in selected]),
                'relative_linf_std': np.std([row['relative_linf'] for row in selected]),
                'mse_mean': np.mean([row['mse'] for row in selected]),
                'mse_std': np.std([row['mse'] for row in selected]),
            })
    time_values = np.array(sorted({row['time'] for row in time_metric_rows}))
    l2_by_time = [
        [row['relative_l2'] for row in time_metric_rows if row['time'] == time_value]
        for time_value in time_values
    ]
    l2_mean = np.array([np.mean(values) for values in l2_by_time])
    l2_std = np.array([np.std(values) for values in l2_by_time])
    fig, ax = plt.subplots()
    ax.set_yscale('log')
    ax.plot(time_values, l2_mean, color='#1b9e77', label='relative L2')
    ax.fill_between(time_values, np.clip(l2_mean - l2_std, 1e-12, None),
                    l2_mean + l2_std, color='#1b9e77', alpha=0.2)
    ax.set_xlabel('Time t')
    ax.set_ylabel('Relative L2 error')
    ax.legend()
    fig.tight_layout()
    plt.savefig(os.path.join(AGG_FIG_DIR, 'time_resolved_relative_l2.pdf'), dpi=300,
